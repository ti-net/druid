...
public class StatViewServlet extends ResourceServlet {
	...
	private static Map<String, String> jmxUrlMap = null;
	...
	public void init() throws ServletException {
		...
		// 获取jmx的连接配置信息
		jmxUrl = readInitParam(PARAM_NAME_JMX_URL);
		String jmxUrls = readInitParam(PARAM_NAME_JMX_URLS);
		if (jmxUrl == null) {
			if (jmxUrls != null) {
				jmxUrlMap = new LinkedHashMap<String, String>();
				String[] jmxUrlArray = jmxUrls.split(",");
				for (int i = 0; i < jmxUrlArray.length; i++) {
					String jmxUrlKey = jmxUrlArray[i].substring(0, jmxUrlArray[i].indexOf('|'));
					String jmxUrlValue = jmxUrlArray[i].substring(jmxUrlArray[i].indexOf('|') + 1);
					jmxUrlMap.put(jmxUrlKey, jmxUrlValue);
					if (i == 0) {
						jmxUrl = jmxUrlValue;
					}
				}
	...
	/**
	 * 程序首先判断是否存在jmx连接地址，如果不存在，则直接调用本地的druid服务；
	...
	protected String process(String url) {
		String resp = null;
		if (jmxUrl == null || url.startsWith("/get-jmx-url.json") || url.startsWith("/reset-jmx-url.json")) {
	...
	public static Map<String, String> getJmxUrls() {
		return jmxUrlMap;
	}

	public static String getCurrentJmxUrl() {
		return jmxUrl;
	}

	public static void resetJmxUrl(String url) {
		if (!url.equals(jmxUrl)) {
			jmxUrl = url;
			conn = null;
		}
	}
